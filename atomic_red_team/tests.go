...
func (t Test) Run(ctx context.Context, atomicsDir string, opts *TestOptions) (*TestResult, error) {
	...
	if len(t.Dependencies) > 0 {
		...
		dependencyResolutionResults, err = t.resolveDependencies(ctx, atomicsDir, inputArguments, opts.CommandOptions)
		if err != nil {
	...
	executedCommand, err := ExecuteCommand(ctx, command, t.Executor.Name, opts.CommandOptions)
	if err != nil {
...
func (t Test) resolveDependencies(ctx context.Context, atomicsDir string, inputArguments map[string]interface{}, opts *CommandOptions) ([]DependencyResolutionResult, error) {
	var results []DependencyResolutionResult
	for _, dependency := range t.Dependencies {
		result, err := dependency.ResolveDependency(ctx, atomicsDir, inputArguments, opts)
		if err != nil {
			return nil, err
		}
...
func (d Dependency) CheckDependency(ctx context.Context, atomicsDir string, inputArguments map[string]interface{}, opts *CommandOptions) (*DependencyResolutionResult, error) {
	description := strings.TrimSpace(d.Description)
	log.Infof("Checking dependency: %s", description)
	executedCommand, met, err := d.checkDependency(ctx, atomicsDir, inputArguments, opts)
	if err != nil {
		return nil, err
	}
	if met {
	...
	result := &DependencyResolutionResult{
		Dependency:       d,
		ExecutedCommands: []ExecutedCommand{*executedCommand},
		Met:              met,
	}
	return result, nil
}

func (d Dependency) checkDependency(ctx context.Context, atomicsDir string, inputArguments map[string]interface{}, opts *CommandOptions) (*ExecutedCommand, bool, error) {
	inputArguments = d.combineArgs(inputArguments)
	command, err := prepareCommand(d.PrereqCommand, atomicsDir, inputArguments)
	if err != nil {
		return nil, false, err
	}
	executedCommand, err := ExecuteCommand(ctx, command, d.ExecutorName, opts)
	if err != nil {
		return nil, false, err
	}
...
func (d Dependency) ResolveDependency(ctx context.Context, atomicsDir string, inputArguments map[string]interface{}, opts *CommandOptions) (*DependencyResolutionResult, error) {
	executedCommands, met, err := d.resolveDependency(ctx, atomicsDir, inputArguments, opts)
	if err != nil {
		return nil, err
	}
	result := &DependencyResolutionResult{
...
func (d Dependency) resolveDependency(ctx context.Context, atomicsDir string, inputArguments map[string]interface{}, opts *CommandOptions) ([]ExecutedCommand, bool, error) {
	var executedCommands []ExecutedCommand
	inputArguments = d.combineArgs(inputArguments)
